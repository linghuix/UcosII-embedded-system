...
- 将当前任务加入等待列表，
- 设置属性OSTCBCur->OSTCBEventPtr，保存任务的等待事件
- 从就绪表中注销，
...
* **如果需要获取mutex的任务 临时提升优先级 比 占有任务优先级 高，那么把占用其的任务提升优先级(从就绪表或者等待列表中去除，将任务优先级设置为临时提升优先级，再按原样恢复)。（防止其他任务插在这两个任务之间，导致优先级反转）**
* OSTCBStat 为mutex等待 和 OSTCBStatPend为OK，并使任务进入等待列表，强制执行调度
* 再度调度会该任务，根据OSTCBStatPend请求返回状态，进行处理
...
#if (OS_EVENT_EN)
    OS_EVENT        *OSTCBEventPtr;  /*Pointer to  event control block任务等待的事件指针*/
#endif
