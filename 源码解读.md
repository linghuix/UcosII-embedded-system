...
* 调用OS_EventTaskWait
* 执行调度.
* 下次任务切换回该任务时，检验OSTCBStatPend，=OS_STAT_PEND_OK表示获取了信号，OS_ERR_PEND_ABORT表示信号获取出错
* OS_STAT_PEND_TO获取超时调用OS_EventTaskRemove从等待列表中删除任务
...
* 执行任务调度
* 任务再次调回来时，判断OSTCBStatPend挂起的状态，如果未成功(时钟节拍给予标志)调用OS_FlagUnlink，返回请求到的信号量集和对应的错误；如果调用成功(OSFlagPost给予标志)，则消费信号量，返回请求到的信号量集
